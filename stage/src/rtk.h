...
/// @define STK_SINCOS(a, sina, cosa) 
/// Set sina and cosa to the sin and cos of a. sina and cosa must be
/// dereferencable variables.
#ifdef HAVE_SINCOS
#define STG_RTK_SINCOS(a, sina, cosa) sincos((a), &(sina), &(cosa));
#else
#define STG_RTK_SINCOS(a, sina, cosa) {\
  sina = sin(a); \
  cosa = cos(a); \
}
#endif
...
// For debugging when the stack is messed up
extern char rtk_last_function[128];

#define RTK_F_INIT { strncpy(rtk_last_function, "none", 127); }
//#define RTK_F { strncpy(rtk_last_function, __PRETTY_FUNCTION__, 127); rtk_last_function[127] = '\0'; }
//#define RTK_XF(f) { strncpy(rtk_last_function, f, 127); rtk_last_function[127] = '\0'; }
#define RTK_F { }
#define RTK_XF(f) { }
...
typedef struct _stg_rtk_canvas_t
{
  ...
  GtkLabel *timeavg_label;
  GtkLabel *status_label;

  GdkColor status_bar_alert_color;
  stg_blinker_t status_bar_blinker;
  ...
  // Flag set to false if we should just skip rendering figures
  gboolean draw_figs;
  ...
  // which mouse button should do what
  int mouse_button_zoom;
  int mouse_button_pan;

  // remember the normal cursor
...
// Set whether any drawing should be done (normally yes)
...
typedef struct _stg_rtk_fig_t
{
  ...
  // A figure may also have some associated object (i.e. a stage model) pointed
  // to by its userdata field, which might also be holding references to
  // that figure. Therefore, you may here set a function that erases
  // those references to avoid dangling pointers. It's called by
  // stg_rtk_fig_destroy before deleting the fig structure. (rh)
  void (*predeletefn)(struct _stg_rtk_fig_t* fig);

  // A name for this figure, used by e.g. Stage to remember what key was used
  // in model datalists, or for diagnostic purposes. (rh)
  // Allocated by strdup in fig_create and destroyed in fig_destroy
  char* name;
...
stg_rtk_fig_t *stg_rtk_fig_create(stg_rtk_canvas_t *canvas, stg_rtk_fig_t *parent, int layer, const char* name );
...
stg_rtk_fig_t *stg_rtk_fig_create_ex(stg_rtk_canvas_t *canvas, stg_rtk_fig_t *parent, 
			     int layer, void* userdata, char* name );
...
// Append the given text to the messages window
// Style is one of:
//   NULL  Plain text
//   "bold"  Bold text
//   "red"  Red text
//   "bold red"  Bold and red
