...
/// @todo add destroy callback that removes property toggles from gui
...
void blobfinder_load( stg_model_t* mod, int worldfile_section_id );
...
void blobfinder_load( stg_model_t* mod, int wf_id )
{
  ...
  bcfg.channel_count = 
    wf_read_int(wf_id, "channel_count", now->channel_count );
  
  bcfg.scan_width = (int)
    wf_read_tuple_float(wf_id, "image", 0, now->scan_width );
  bcfg.scan_height = (int)
    wf_read_tuple_float(wf_id, "image", 1, now->scan_height );	    
  bcfg.range_max = 
    wf_read_length(wf_id, "range_max", now->range_max );
  bcfg.pan = 
    wf_read_tuple_angle(wf_id, "ptz", 0, now->pan );
  bcfg.tilt = 
    wf_read_tuple_angle(wf_id, "ptz", 1, now->tilt );
  bcfg.zoom =  
    wf_read_tuple_angle(wf_id, "ptz", 2, now->zoom );
  ...
  for( ch = 0; ch<bcfg.channel_count; ch++ )
    { 
      const char* colorstr = wf_read_tuple_string( wf_id, "channels", ch, NULL );
...
{
  stg_blob_return_t* br = 
    stg_model_get_property_fixed( found, 
				  "blob_return", sizeof(stg_blob_return_t));
  
  if( br && *br && !stg_model_is_related( finder, found ))
    return 1;
...
int blobfinder_update( stg_model_t* mod )
{
  ...
  for( s = 0; s < cfg->scan_width; s++)
      ...
      if( ent )
	{
	  ...
	  stg_color_t *hiscol = 
	    stg_model_get_property_fixed( ent, "color", sizeof(stg_color_t));
