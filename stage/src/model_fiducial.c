...
/// @todo add destroy callback taht removes property toggles from gui
...
const double STG_F()IDUCIAL_WATTS = 10.0;
...
void fiducial_load( stg_model_t* mod, int wf_id )
{
  
  if( wf_property_exists( wf_id, "range_min" ) ||
      wf_property_exists( wf_id, "range_max" ) ||
      wf_property_exists( wf_id, "range_max_id") ||
      wf_property_exists( wf_id, "fov" ) )
    {
	...
	stg_model_get_property_fixed( mod, "fiducial_cfg", sizeof(stg_fiducial_config_t));
      ...
      cfg.fov = 
	wf_read_angle(wf_id, "fov", now ? now->fov :STG_DEFAULT_FIDUCIAL_FOV );
      
      cfg.min_range = 
	wf_read_length(wf_id, "range_min", now ? now->min_range : STG_DEFAULT_FIDUCIAL_RANGEMIN );
      
      cfg.max_range_anon = 
	wf_read_length(wf_id, "range_max", now ? now->max_range_anon :STG_DEFAULT_FIDUCIAL_RANGEMAXANON );
      
      cfg.max_range_id = 
	wf_read_length(wf_id, "range_max_id", now ? now->max_range_id :STG_DEFAULT_FIDUCIAL_RANGEMAXID );
...
void model_fiducial_check_neighbor( gpointer key, gpointer value, gpointer user )
{
  ...
  stg_matrix_lock(him->world->matrix);
  ...
  itl_destroy( itl );
  stg_matrix_unlock(him->world->matrix);
...
{  
  ...
  stg_rtk_fig_color_rgb32( fig, stg_lookup_color( STG_F()IDUCIAL_COLOR ) );
