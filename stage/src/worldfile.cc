...
//#define DEBUG
#define DUMP_WORLDFILE false // for debugging
...
///////////////////////////////////////////////////////////////////////////
// File path tests for portability 
  #ifdef _WIN32
    ...
    #define PATHSEPSTR "\\"
  #else
    #define IS_ABSOLUTE_PATH(path) (path[0] == '/' || path[0] == '~')
    #define PATHSEPCHAR '/'
    #define PATHSEPSTR "/"
  #endif
...
bool CWorldFile::Load(const char *filename, bool echo)
{
  ...
  if (DUMP_WORLDFILE || ReadInt(0, "test", 0) != 0)
  {
...
bool CWorldFile::LoadTokens(FILE *file, int include, bool echo)
{
  int ch;
  int line;
  char token[256];
  ...
  while (true)
  {
    ...
    {
      ungetc(ch, file);
      if (!LoadTokenComment(file, &line, include, echo))
        return false;
    }
    else if (isalpha(ch))
    {
      ungetc(ch, file);
      if (!LoadTokenWord(file, &line, include, echo))
        return false;
    }
    else if (strchr("+-.0123456789", ch))
    {
      ungetc(ch, file);
      if (!LoadTokenNum(file, &line, include, echo))
        return false;
    }
    else if (isblank(ch))
    {
      ungetc(ch, file);
      if (!LoadTokenSpace(file, &line, include, echo))
        return false;
    }
    else if (ch == '"')
    {
      ungetc(ch, file);
      if (!LoadTokenString(file, &line, include, echo))
        return false;
    }
    else if (strchr("(", ch))
    {
    ...
    else if ( 0x0d == ch )   // Carriage Return
    {
      ch = fgetc(file);
      if ( 0x0a != ch ) 
        ungetc(ch, file);
...
bool CWorldFile::LoadTokenComment(FILE *file, int *line, int include, bool echo)
{
  char token[256];
  int len;
  int ch;
  ...
  while (true)
  {
    ...
    else if ( 0x0a == ch || 0x0d == ch ) // newline, CR
    {
      if(echo) putchar(ch);
      ungetc(ch, file);
      AddToken(TokenComment, token, include);
...
bool CWorldFile::LoadTokenWord(FILE *file, int *line, int include, bool echo)
{
  char token[256];
  int len;
  int ch;
  ...
  while (true)
  {
    ...
    else
    {
      if (strcmp(token, "include") == 0)
      {
        ungetc(ch, file);
        AddToken(TokenWord, token, include);
        if (!LoadTokenInclude(file, line, include, echo))
          return false;
      }
      else
      {
...
bool CWorldFile::LoadTokenInclude(FILE *file, int *line, int include, bool echo)
{
  int ch;
  const char *filename;
  char *fullpath;
  ...
  else if (IS_ABSOLUTE_PATH(this->filename))
  {
  ...
  stg_print_msg("including world file \"%s\"...", filename);
  ...
  // Read tokens from the file
  if (!LoadTokens(infile, include + 1, echo))
  {
    //DumpTokens();
    free(fullpath);
    fclose(infile);
...
bool CWorldFile::LoadTokenNum(FILE *file, int *line, int include, bool echo)
{
  char token[256];
  int len;
  int ch;
...
bool CWorldFile::LoadTokenString(FILE *file, int *line, int include, bool echo)
{
  int ch;
  int len;
  char token[256];
  ...
  while (true)
  {
    ...
    if ( EOF == ch || 0x0a == ch || 0x0d == ch ) // eof, newline, CR
    {
...
bool CWorldFile::LoadTokenSpace(FILE *file, int *line, int include, bool echo)
{
  int ch;
  int len;
  char token[256];
  ...
  for (i = *index + 1; i < this->token_count; i++)
  {
    ...
    switch (token->type)
    {
      ...
      case TokenCloseEntity:
        ...
        if (count == 0)
        {
          // Use the definition as a macro for future file parsing:
          AddMacro(macroname, entityname, *line, starttoken, i);

          // DEBUG rh PrintWorldTypeDef(macroname, entityname, *line, starttoken, i);
...
bool CWorldFile::ParseTokenWord(int entity, int *index, int *line, int tab_count/* = 0*/)
{
  int i;
  CToken *token;
  ...
  for (i = *index + 1; i < this->token_count; i++)
  {
    ...
    switch (token->type)
    {
      ...
      case TokenEOL:
        ...
        break;
      case TokenOpenEntity:
        return ParseTokenEntity(entity, index, line, NULL, tab_count+1);  // TODO: remove ", NULL, tab_count+1"
      case TokenNum:
      case TokenString:
      case TokenOpenTuple:
...
int CWorldFile::CreateEntity(const char *type, int parentent)
{
  int entity;

  // We have to check if it's a macro before trying to call ParseTokenEntity,
  // since it needs "worldfile data" to read from, which non-macro entities
  // created by this function can't have. 

  int macro = LookupMacro(type);

  if(macro >= 0)
  {
    entity = this->entity_count;
  ...
  {
    entity = AddEntity(parentent, type, type);
  }
...
bool CWorldFile::ParseTokenEntity(int entity, int *index, int *line, const char *macro_name, int tab_count/* = 0*/)
{
  int i;
  int macro;
  int name;
  ...
  if (macro >= 0)
  {
    ...
    int mindex = this->macros[macro].starttoken;
    int mline = this->macros[macro].line;
    if (!ParseTokenEntity(entity, &mindex, &mline, macro_name?macro_name:GetTokenValue(name), tab_count+1))
      return false;
    entity = nentity;
    ...
    for (i = *index + 1; i < this->token_count; i++)
    {
      ...
      switch (token->type)
      {
        case TokenOpenEntity:
          break;
        case TokenWord:
          if (!ParseTokenWord(entity, &i, line, tab_count))
            return false;
          break;
        case TokenCloseEntity:
          *index = i;
        ...
        case TokenEOL:
          ...
          break;
        default:
          PARSE_ERR("syntax error 3 (unexpected character while parsing keyword)", *line);
          return false;
      }
    }
    PARSE_ERR("missing ')'", *line);
  ...
  else
  {
    for (i = *index + 1; i < this->token_count; i++)
    {
      ...
      {
        case TokenOpenEntity:
          entity = AddEntity(entity, GetTokenValue(name), macro_name?macro_name:GetTokenValue(name));
          break;
        case TokenWord:
          if (!ParseTokenWord(entity, &i, line))
            return false;
...
void CWorldFile::DumpMacros()
{
  ...
  for (int i = 0; i < this->macro_count; i++)
  {
    ...
    printf("## %d ## name: %s.  section/entity name: %s. tokens:\n\t", LookupMacro(macro->macroname), macro->macroname, macro->entityname);
...
///////////////////////////////////////////////////////////////////////////
// Add a entity
int CWorldFile::AddEntity(int parent, const char *type, const char *immediate_type)
{
  ...
  this->entities[entity].immediate_type = immediate_type;
  this->entity_count++;
...
// Give 0 for start to start search at the first entity in the world. Returns -1 if there are no more.
int CWorldFile::GetNextChildEntity(int parent, int start)
{
  if(start < 0) return -1;
  for(int i = start; i < entity_count; i++)
  {
    if(i != parent && entities[i].parent == parent) return i;
  }
  return -1;
}
...
const char *CWorldFile::GetEntityImmediateType(int entity)
{
  if(entity < 0 || entity >= this->entity_count) return NULL;
  return this->entities[entity].immediate_type;
}
                                  ...
                                  int index, double value)
{
  ...
  if (property < 0)
    return value;
  const char* p = GetPropertyValue(property, index);
  if(!p)
      return value;
  return atof(p);
}
                                   ...
                                   int index, double value)
{
  ...
  const char* p = GetPropertyValue(property, index);
  if(!p) return value;
  return atof(p) * this->unit_length;
}
...
void CWorldFile::WriteTupleInt(int entity, const char *name, int index, int value)
{
  char str[64];
  snprintf(str, sizeof(str), "%d", value);
  WriteTupleString(entity, name, index, str);
}



//////////////////////////////////////////////////////////////////////////
// Print info about a model type definition. 
// This info could be stored for possible future use by the program
// in worlfile_typetable in world.c
void CWorldFile::PrintWorldTypeDef(const char *macroname, const char *entityname,
  int line, int starttoken, int endtoken)
{
  //GDataList *props = g_hash_table_new(g_str_hash, g_str_equal);
  CToken *token;
  for(int i = starttoken; i <= endtoken; i++)
  {
    token = this->tokens + i; // get token

    // Get property tokens, skip the others
    switch(token->type)
    {
      case TokenOpenEntity:
        printf("** PrintWorldTypeDef: %s macro has nested entity:...", macroname);
        break;
      case TokenWord:
        printf("** PrintWorldTypeDef: %s macro has word token[%d] = \"%s\".\n", macroname, i, GetTokenValue(i));
        break;
      case TokenNum:
      case TokenString:
        printf("** PrintWorldTypeDef: %s macro has value token[%d] = \"%s\".\n", macroname, i, GetTokenValue(i));
        break;
      case TokenOpenTuple:
        printf("** PrintWorldtypeDef: %s macro value is a tuple...", macroname);
        break;
      default:
  ...
  //stg_register_user_type(...);  // TODO
}


const char *CWorldFile::GetMacroName(int macro) {
  if(macro >= macro_count) return NULL;
  return macros[macro].macroname;
}

const char *CWorldFile::GetMacroEntityName(int macro) {
  if(macro >= macro_count) return NULL;
  return macros[macro].entityname;
}

void CWorldFile::SetEntityIdForModelType(char *typestr, stg_id_t entity_id)
{
  myModelTypeToWorldIdMap[typestr] = entity_id;

  // debug print
  //printf("myModelTypeToWorldIdMap: size (%d)\n", myModelTypeToWorldIdMap.size());
  //for(std::map<char *, stg_id_t>::iterator modTypeMap_it = myModelTypeToWorldIdMap.begin(); modTypeMap_it != myModelTypeToWorldIdMap.end(); modTypeMap_it++)
    //printf("myModelTypeToWorldIdMap: key (%s): val (%d)\n", modTypeMap_it->first, modTypeMap_it->second);
}

stg_id_t CWorldFile::FindEntityIdForModelType(char *typestr)
{
  std::map<char *, stg_id_t>::iterator modType_find = myModelTypeToWorldIdMap.find(typestr);
  if(modType_find != myModelTypeToWorldIdMap.end())
    return modType_find->second;
