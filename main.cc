  ...
  void (*loop_callback)())
{
  ...
  // include PioneerRobotModels.world.inc if it exists, or explain the error if  not.
  char modelfile[MAX_PATH_LEN];
  snprintf(modelfile, MAX_PATH_LEN, "%s%cPioneerRobotModels.world.inc", libdir, PATHSEPCH);
  if(ArUtil::findFile(modelfile))
    fprintf(world_fp, "include \"%s%cPioneerRobotModels.world.inc\"\n", libdir, PATHSEPCH);
  else
  {
    char msg[MAX_PATH_LEN+256];
    snprintf(msg, MAX_PATH_LEN+256, "Could not find model definition file:\n\t%s\n\nClick Continue to proceed with no predefined robot models (see program log output for further warnings and errors).\n\nEither (re-)install AMRISim or set 'AMRISIM' environment variable to location of models file.", modelfile);
    stg_gui_error_dialog("Warning", msg, ERR_MODELFILE);
  }
